...
> **Актуальное состояние (август 2026).** Текст ниже местами устарел.
>
> - Хранилище — **Cloudflare R2** (Supabase поддерживается как запасной вариант).
> - Ограничения по **разрешению нет** — 100+ мегапикселей принимаются.
> - По весу: локально — до 2 ГБ, на Vercel — 4 МБ (жёсткий лимит платформы).
>   Тяжёлые файлы админка автоматически подгоняет под лимит, сохраняя разрешение.
> - Конвертация в AVIF — качество 90 (раньше был lossless, из-за него файлы раздувались в 7 раз
>   и загрузка падала по таймауту).
